...
export default function DonHang() {
    ...
    const filteredOrders = orders.filter((order) => order.id.toString().includes(searchTerm));
    ...
    const handleViewOrderDetails = (orderId: number) => {
        ...
        (
            document.getElementById(`order_detail_modal_${orderId}`) as HTMLDialogElement
        )?.showModal();
    };
    ...
    const handleSubmitReview = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const [response, error] = await productReviewService.createReview(
                                    ...
                                    <div className="avatar">
                                        <div className="w-16 h-16 rounded">
                                            {reviewItem.product.product_images?.length > 0 ? (
                                                <img
                                                    src={
                                                        reviewItem.product.product_images[0]
                                                            ?.image_url
                                                    }
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm">
                                            <span className="badge badge-outline badge-sm">
                                                {reviewItem.variant.sku ||
                                                    `#${reviewItem.variant.id}`}
                                            </span>
                                        </p>
                                    </div>
                                </div>
